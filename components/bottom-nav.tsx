...
export function BottomNav({ active, onChange }: { active: TabKey; onChange: (tab: TabKey) => void }) {
  return (
    <div className="pointer-events-none sticky bottom-0 z-30 w-full px-3 pb-[calc(env(safe-area-inset-bottom)+14px)] pt-2 sm:pb-[calc(env(safe-area-inset-bottom)+1.25rem)]">
      <nav
        ...
        {tabs.map((t) => {
          ...
          return (
            <button
              ...
              aria-current={isActive ? "page" : undefined}              className="group relative flex min-w-0 flex-1 flex-col items-center justify-center rounded-[1.3rem] py-2 transition-transform duration-150 active:scale-90 sm:py-2.5">
              {/* Active capsule — only behind the selected tab */}
              <span
                className={cn(
                  "absolute left-1/2 top-1 h-[38px] w-[46px] -translate-x-1/2 rounded-2xl transition-all duration-300 ease-out sm:h-[2.75rem] sm:w-[3.5rem]",
                  isActive ? "scale-100 bg-primary/16 opacity-100" : "scale-75 opacity-0",
                )}
              />
              <Icon
                className={cn(
                  "relative h-[20px] w-[20px] transition-colors duration-200 sm:h-6 sm:w-6",
              ...
              <span
                className={cn(
                  "relative mt-0.5 text-[9px] font-bold leading-none tracking-tight transition-colors duration-200 sm:text-[0.6875rem]",
